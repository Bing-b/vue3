...
// 动态统计当前示例各类lable数量
const labelStore: ComputedRef<{ id: number, text: string, color: string, borderColor: string, num: number }[]> = computed(() => {
  if (annotator.value === null) {
    return [];
  }
  const result = [];
  const labelCategories: { id: number, text: string, color: string, borderColor: string, num?: number }[] = [];
  for (const [_, category] of annotator.value!.store.labelCategoryRepo) {
