(function () {
  ...
  const ExtendMethods = {
    ...
    _getTileGeometry: function (x, y, z, skipIntersectionCheck) {
      ...
      const cacheID = x + ':' + y + ':' + z;
      const zCoeff = Math.pow(2, z);
      let parentState;
      const cache = this._boundaryCache;
      ...
      const tileBbox = new L.Bounds(new L.Point(x * ts / zCoeff, y * ts / zCoeff), new L.Point((x + 1) * ts / zCoeff, (y + 1) * ts / zCoeff));
    ...
    _drawTileInternal: function (canvas, tilePoint, url, callback) {
      ...
      const ts = this.options.tileSize;
      const tileX = ts * tilePoint.x;
      const tileY = ts * tilePoint.y;
      const zCoeff = Math.pow(2, zoom);
    ...
    _updateAttribution: function () {
      ...
      const mercBounds = L.bounds(this._map.project(mapBounds.getSouthWest(), 0), this._map.project(mapBounds.getNorthEast(), 0));
      const state = this._getClippedGeometry(geom, mercBounds);
