<template>
  <div>
    <p> <span class="text-[14px]"></span>
      （ <span class="text-[red]">自动上传</span>
      、拖拽上传、拖拽排序、设置请求头、上传进度、大图预览、多选文件、最大文件数量、文件类型限制、文件大小限制、删除文件）</p>

    <el-upload class="pure-upload" v-model:file-list="fileList" drag multiple list-type="picture-card"
      accepet="image/jpeg,image/png,image/svg" action="https://run.mocky.io/v3/3aa761d7-b0b3-4a03-96b3-6168d4f7467b"
      :limit="3" :header="{ Authorization: 'eyJhbGciOiJIUzUxMiJ9.admin' }" :on-exceed="onExceed"
    ...
    <el-image-viewer v-if="dialogVisible" :initialIndex="curOpenImgIndex" :url-list="urlList" :zoom-rate="1.2"
      :max-scale="7" :min-scale="0.2" @close="dialogVisible = false" @switch="index => (curOpenImgIndex = index)" />
