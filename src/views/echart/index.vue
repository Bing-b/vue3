...
const bindListen = (myChart) => {
  ...
  myChart.on('click', function (params) {
    ...
    const isSelected = !option.value.series[params.seriesIndex].pieStatus.selected;
    const isHovered = option.value.series[params.seriesIndex].pieStatus.hovered;
    const k = option.value.series[params.seriesIndex].pieStatus.k;
    const startRatio = option.value.series[params.seriesIndex].pieData.startRatio;
    const endRatio = option.value.series[params.seriesIndex].pieData.endRatio;
    // 如果之前选中过其他扇形，将其取消选中（对 option 更新）
    if (selectedIndex !== '' && selectedIndex !== params.seriesIndex) {
      option.value.series[selectedIndex].parametricEquation = getParametricEquation(option.value.series[
        selectedIndex].pieData
        .startRatio, option.value.series[selectedIndex].pieData.endRatio, false, false, k, option.value.series[
          selectedIndex].pieData
        .value);
      option.value.series[selectedIndex].pieStatus.selected = false;
    }
    // 对当前点击的扇形，执行选中/取消选中操作（对 option 更新）
    option.value.series[params.seriesIndex].parametricEquation = getParametricEquation(startRatio, endRatio,
      isSelected,
      isHovered, k, option.value.series[params.seriesIndex].pieData.value);
  ...
  myChart.on('mouseover', function (params) {
    ...
    if (hoveredIndex === params.seriesIndex) {
      ...
      if (hoveredIndex !== '') {
        ...
        option.value.series[hoveredIndex].parametricEquation = getParametricEquation(startRatio, endRatio,
          isSelected,
          isHovered, k, option.value.series[hoveredIndex].pieData.value);
      ...
      if (params.seriesName !== 'mouseoutSeries' && params.seriesName !== 'pie2d') {
        ...
        option.value.series[params.seriesIndex].parametricEquation = getParametricEquation(startRatio, endRatio,
          isSelected, isHovered, k, option.value.series[params.seriesIndex].pieData.value + 5);
...
const init = () => {
  ...
  // 传入数据生成 option
  option.value = getPie3D(optionData.value, 0);
  myChart.setOption(option.value);
